...
public class SensorController {
    ...
    @RequestMapping("/ShowSensorChart.do")
    public String ShowSensorChart(Model model, HttpServletRequest request){
        String deviceID = request.getQueryString();
        List<SensorInfoList> list = sensorInfoMapper.findDataByUser(deviceID);
        model.addAttribute("sensorinfosList", list);
        model.addAttribute("deviceID", deviceID);
        return "sensor/sensorChart";
    }
}
